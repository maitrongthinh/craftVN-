# Common Issues
- `Error: connect ECONNREFUSED`: Your bot failed to connect with the Minecraft server. Most likely due to:
...
- `My brain disconnected, try again`: Something is wrong with the LLM API connection. You may have the wrong API key, exceeded your rate limits, or other issues. Check the program outputs for more details.
