...
export const log = (agentName, msg) => {
    ...
    console.error(msg);
    try { sendOutputToServer(agentName || 'system', msg); } catch (err) { console.warn("[ConnectionHandler] Cannot send output to server:", err.message); }
};
...
export function parseKickReason(reason) {
    ...
    } catch (err) { console.warn("[ConnectionHandler] Error parsing JSON kick reason:", err.message); }
