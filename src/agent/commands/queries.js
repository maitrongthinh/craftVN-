...
export const queryList = [
    ...
    {
        name: '!askMemory',
        description: 'Search spatial memory for remembered locations or chests. Can query block types (e.g. "diamond_ore") or dropped/chest items (e.g. "iron_ingot", "diamond").',
        params: {
            'query': { type: 'string', description: 'Item or block to search for (e.g. "diamond", "chest", "iron").' }
        },
        perform: function (agent, query) {
            ...
            let result = agent.memory_bank.findNearest(query, pos, dimension);
            let chestResult = agent.memory_bank.findItemInChests(query, pos, dimension);
            
            if (chestResult) {
                if (result.includes('No remembered locations') || result.includes('Invalid query')) {
                    return chestResult;
                } else {
                    return result + '\n\n' + chestResult;
                }
