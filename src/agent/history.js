...
export class History {
    constructor(agent) {
        ...
        // Debounce save() to prevent excessive I/O
        this._saveTimer = null;
        this._saveDebounceMs = 5000; // Coalesce saves within 5 seconds
    }
    ...
    async forceCompressMemory() {
        if (this.turns.length > 0) {
            let chunk = this.turns.splice(0, this.turns.length);
            if (this.pinnedMessage && !chunk.some(t => t.content === this.pinnedMessage.content)) {
                chunk.unshift(this.pinnedMessage);
            }
            await this.summarizeMemories(chunk);
            await this.appendFullHistory(chunk);
        }
    ...
    async save() {
        // Debounced: coalesce multiple rapid save() calls into one
        if (this._saveTimer) {
