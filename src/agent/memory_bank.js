...
export class MemoryBank {
	...
	initSpatialMemory(botName) {
		...
		this._chestMemoryFp = `./bots/${botName}/chest_memory.json`;
	...
	saveSpatialMemory() {
		...
		try {
			...
			if (this._chestMemoryFp) {
				writeFileSync(this._chestMemoryFp, JSON.stringify(this.chestMemory, null, 2), 'utf8');
			}
		} catch (err) {
	...
	loadSpatialMemory() {
		...
		try {
			...
			if (this._chestMemoryFp && existsSync(this._chestMemoryFp)) {
				const chestData = JSON.parse(readFileSync(this._chestMemoryFp, 'utf8'));
				if (chestData && typeof chestData === 'object') {
					this.chestMemory = chestData;
					console.log(`Loaded ${Object.keys(this.chestMemory).length} chest memory entries.`);
				}
			}
		} catch (err) {
	...
	/**
	 * Save the contents of a chest to memory.
	 */
	rememberChest(x, y, z, dimension, itemsDict) {
		if (!this.chestMemory) this.chestMemory = {};
		const key = `${dimension}_${Math.floor(x)}_${Math.floor(y)}_${Math.floor(z)}`;
		this.chestMemory[key] = {
			position: [Math.floor(x), Math.floor(y), Math.floor(z)],
			dimension: dimension,
			items: itemsDict,
			last_updated: Date.now()
		};
		// Rely on agent.js auto-save logic to persist this to disk
	}

	/**
	 * Find an item in all remembered chests.
	 */
	findItemInChests(keyword, botPosition, dimension = 'minecraft:overworld') {
		if (!this.chestMemory) return null;
		const searchItem = keyword.toLowerCase();
		let matches = [];
		for (const [key, chest] of Object.entries(this.chestMemory)) {
			if (chest.dimension !== dimension && dimension !== 'all') continue;
			for (const [itemName, count] of Object.entries(chest.items)) {
				if (itemName.toLowerCase().includes(searchItem) && count > 0) {
					const dx = chest.position[0] - botPosition.x;
					const dy = chest.position[1] - botPosition.y;
					const dz = chest.position[2] - botPosition.z;
					const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
					matches.push({ chest, itemName, count, dist });
				}
			}
		}
		...
		for (let i = 0; i < Math.min(5, matches.length); i++) {
			const m = matches[i];
			results.push(`- ${m.count}x ${m.itemName} at chest x=${m.chest.position[0]}, y=${m.chest.position[1]}, z=${m.chest.position[2]} (${m.dist.toFixed(1)} blocks away)`);
		}
		return `Found "${searchItem}" in remembered chests:\n` + results.join('\n');
	}
}