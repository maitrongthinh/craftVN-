...
export async function craftRecipe(bot, itemName, num = 1) {
    ...
    // Tự động craft planks từ log nếu thiếu planks khi chế table hoặc gậy
    if (!recipes || recipes.length === 0) {
        if (itemName === 'crafting_table') {
            let hasLogs = bot.inventory.items().find(item => item.name.endsWith('_log'));
            if (hasLogs) {
                log(bot, `Auto-crafting planks from ${hasLogs.name} to make crafting_table...`);
                let plankName = hasLogs.name.replace('_log', '_planks');
                let success = await craftRecipe(bot, plankName, 1);
                if (success) recipes = bot.recipesFor(mc.getItemId(itemName), null, 1, null);
            }
        } else if (itemName === 'stick') {
            let hasPlanks = bot.inventory.items().find(item => item.name.endsWith('_planks'));
            let hasLogs = bot.inventory.items().find(item => item.name.endsWith('_log'));
            if (!hasPlanks && hasLogs) {
                log(bot, `Auto-crafting planks from ${hasLogs.name} to make stick...`);
                let plankName = hasLogs.name.replace('_log', '_planks');
                let success = await craftRecipe(bot, plankName, 1);
                if (success) recipes = bot.recipesFor(mc.getItemId(itemName), null, 1, null);
            }
    ...
    try {
        ...
        log(bot, `Crafting failed due to desync or missing items: ${err.message}`);
...
export async function attackNearest(bot, mobType, kill = true) {
    ...
    if (mobType === 'drowned' || mobType === 'cod' || mobType === 'salmon' || mobType === 'tropical_fish' || mobType === 'squid')
        bot.modes.pause('anti_drown'); // so it can dive underwater without fighting the surface reflex
...
export async function defendSelf(bot, range = 9) {
    ...
    // Khởi tạo Movements 1 lần duy nhất cho toàn bộ hàm để chống memory leak
    const combatMovements = new pf.Movements(bot);

    while (enemy) {
        await equipHighestAttack(bot);
        if (bot.entity.position.distanceTo(enemy.position) >= 4 && enemy.name !== 'creeper' && enemy.name !== 'phantom') {
            try {
                bot.pathfinder.setMovements(combatMovements);
...
export async function collectBlock(bot, blockType, num = 1, exclude = null) {
    ...
    for (let i = 0; i < num; i++) {
        ...
        try {
            ...
            if (isLiquid) {
                ...
                await Promise.race([
                    ...
                    new Promise((resolve, reject) => setTimeout(() => {
                        bot.pathfinder.stop(); // Giết đường dẫn nếu kẹt (memory & CPU savior)
                        reject(new Error('Collect block timeout'));
                    }, 40000)) // Rút ngắn từ 60s xuống 40s
                ]);
                success = true;
            }
            if (success)
            ...
            bot.pathfinder.stop(); // Dọn dẹp task treo 
...
export async function pickupNearbyItems(bot) {
    ...
    // Tái sử dụng movements để Pathfinder khỏi leak bộ nhớ
    ...
    while (nearestItem) {
        ...
        try {
            ...
            log(bot, 'Item pickup pathfinding error.');
            break;
        }
        await new Promise(resolve => setTimeout(resolve, 200));
...
export async function putInChest(bot, itemName, num = -1) {
    ...
    let itemsDict = {};
    for (let i of items) {
        itemsDict[i.name] = (itemsDict[i.name] || 0) + i.count;
    }
    if (bot.mindcraft_agent && bot.mindcraft_agent.memory_bank) {
        bot.mindcraft_agent.memory_bank.rememberChest(chest.position.x, chest.position.y, chest.position.z, bot.game.dimension, itemsDict);
    }
...
export async function takeFromChest(bot, itemName, num = -1) {
    ...
    let finalItems = chestContainer.containerItems();
    let itemsDict = {};
...
export async function viewChest(bot) {
    ...
    for (let item of items) {
        itemsDict[item.name] = (itemsDict[item.name] || 0) + item.count;
    }
    if (bot.mindcraft_agent && bot.mindcraft_agent.memory_bank) {
        bot.mindcraft_agent.memory_bank.rememberChest(chest.position.x, chest.position.y, chest.position.z, bot.game.dimension, itemsDict);
    }
...
export async function avoidEnemies(bot, distance = 16) {
     ...
     **/
    // Removed bot.modes.pause('self_preservation') so the bot can still react to lava/fire while escaping!
